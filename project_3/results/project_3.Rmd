...
The main purpose of this project is to test whether client demographics have an effect on their length of stay in UMD. Specifically, we are interested in the effects of age, gender, race, ethnicity and veteran status. The following table presents ranges for these covariates:
...
We will first plot the correlation between duration in UMD and different covariates. Then, we will use hypothesis tests to test each covariate's effect.
...
ggplot(data=duration, aes(x=duration)) +
  ...
  facet_wrap(~Gender, scales='free_y', strip.position='bottom') +
  theme_classic() +
...
ggplot(data=duration, aes(x=duration)) +
  geom_histogram(fill='blue') +
  facet_wrap(~Race, scales='free_y', strip.position='bottom') +
  theme_classic() +
  labs(title='Race') +
  theme(plot.title = element_text(hjust = 0.5, size=24),
        axis.title=element_text(size=20),
...
ggplot(data=duration, aes(x=duration)) +
  geom_histogram(fill='blue') +
  facet_wrap(~Ethnicity, scales='free_y', strip.position='bottom') +
  theme_classic() +
...
ggplot(data=duration, aes(x=duration)) +
  geom_histogram(fill='blue') +
  facet_wrap(~VeteranStatus, scales='free_y', strip.position='bottom') +
  theme_classic() +
  theme(strip.text = element_text(size=16)) +
...
For our model, let 'Female', 'Hispanic/Latino', 'American Indian or Alaska Native', 'No' be reference groups for gender, ethnicity, race and veteran status respectively. The summary of Gamma regression is as follows:

```{python echo=FALSE, comment=NA}
from statsmodels.genmod.generalized_linear_model import GLMResults
model = GLMResults.load("gamma_regression.pickle")
model.summary()
```

As we can see, under 0.05 significance level, race doesn't have effects on duration. Moreover, gender, ethnicity and veteran status have effect on duration. Since our link function is an increasing function, specifically,

* Male has negative effect on duration comparing to female.

* Non-Hispanic/Non-Latino have positive effect on duration comparing to Hispanic/Latino.

* Veterans has negative effect on duration comparing to those who are not veterans.

* Age has positive effect on duration. In other words, older clients will stay longer.

#### Conclusion

We have tested whether client demographics have an effect on their length of stay in UMD. Here are some findings:

* Race doesn't have effects on duration.

* Female and Trans Female will stay in UMD longer than Male.

* Non-Hispanic/Non-Latino will generally stay longer in UMD than Hispanic/Latino.

* Veterans will generally stay shorter in UMD than those who are not veterans.

* Older clients will generally stay longer in UMD.
